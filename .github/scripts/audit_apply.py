...
replace(
    p,
    '''use windows::Win32::Graphics::Gdi::{\n    EnumDisplayMonitors, GetMonitorInfoW, HDC, HMONITOR, MONITORINFO, MONITORINFOEXW, MONITORINFOF_PRIMARY,\n};\n''',
...
replace(
    p,
    '''    /// The display device name (for example `\\\\.\\DISPLAY2`). Unlike the\n    /// HMONITOR handle this stays stable when Windows rebuilds monitor handles,\n    /// so explicit index selections can remain attached to the same display\n    /// for the lifetime of this process even if enumeration order changes.\n    pub name: String,\n''',
    '''    /// The GDI display device name (for example `\\\\.\\DISPLAY2`). Useful\n    /// for diagnostics but not durable enough to persist as monitor identity.\n    pub name: String,\n    /// `GUID_DEVINTERFACE_MONITOR` device-interface path. Windows registers it\n    /// per monitor; unlike enumeration position and the GDI DISPLAY number it\n    /// can be used to re-find the same monitor after an enumeration reorder.\n    pub stable_id: Option<String>,\n''',
)
# Add helper before enum callback.
insert_before(
    p,
    "unsafe extern \"system\" fn enum_monitor_proc(\n",
    '''fn monitor_interface_id(gdi_name: &str) -> Option<String> {\n    let wide: Vec<u16> = gdi_name.encode_utf16().chain(std::iter::once(0)).collect();\n    let mut device = DISPLAY_DEVICEW {\n        cb: std::mem::size_of::<DISPLAY_DEVICEW>() as u32,\n        ..Default::default()\n    };\n    // EDD_GET_DEVICE_INTERFACE_NAME = 0x1. It puts the per-monitor\n    // GUID_DEVINTERFACE_MONITOR path in DeviceID.\n    if !unsafe { EnumDisplayDevicesW(PCWSTR(wide.as_ptr()), 0, &mut device, 0x1) }.as_bool() {\n        return None;\n    }\n    let end = device\n        .DeviceID\n        .iter()\n        .position(|&ch| ch == 0)\n        .unwrap_or(device.DeviceID.len());\n    if end == 0 {\n        return None;\n    }\n    Some(String::from_utf16_lossy(&device.DeviceID[..end]))\n}\n\n''',
)
# Ensure PCWSTR import exists.
if "use windows::core::PCWSTR;" not in Path(p).read_text(encoding="utf-8"):
    insert_before(p, "use windows::Win32::Foundation", "use windows::core::PCWSTR;\n")
replace(
    p,
...
# Fake display fixtures need the new field; one canonical helper covers all current tests.
replace(
    p,
...
# Main-window config mutations refresh managed identity metadata before save.
...
# When first enabling a separate Compact slot from defaults, copy identity metadata too.
replace(
    p,
...
# Startup performs an additive best-effort migration before any overlay snapshot exists.
p = "src/main.rs"
replace(
    p,
