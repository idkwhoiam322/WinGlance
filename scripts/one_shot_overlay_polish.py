...
/// Directional edge highlight traced on the pill's own boundary"""
    ...
    replace_comment = """    // Art tile: left-aligned like the expanded pill (inset + padding),
    ...
    replace_comment_new = """    // Art tile: left-aligned like the expanded pill (inset + padding),
    // vertically centered on the row. This is the only place the compact
    // art is drawn — `draw_pixels` skips its art arms in compact mode, so
    ...
    if text.count(replace_comment) != 1:
        raise SystemExit(f"render.rs: compact comment count {text.count(replace_comment)}")
    text = text.replace(replace_comment, replace_comment_new)
